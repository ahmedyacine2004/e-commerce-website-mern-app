...
export default function GenericTable({
  ...
  const openFilterMenu = (e, col) =>
    col.filter && (setMenuAnchor(e.currentTarget), setActiveColumn(col));
  ...
  const searchedData = useMemo(() => {
    if (!search || !serachMetric) return data;

    const metrics = Array.isArray(serachMetric) ? serachMetric : [serachMetric];
  ...
  const filteredData = useMemo(() => {
    return searchedData.filter((row) =>
      columns.every((col) => {
        ...
        let cellValue = col.filterValue
          ? col.filterValue(row[col.accessor], row)
          : row[col.accessor];
  ...
  const groupedPaginatedData = useMemo(() => {
    return Object.fromEntries(
      Object.entries(groupedData)
        .map(([k, rows]) => [k, rows.filter((r) => paginatedRows.includes(r))])
  ...
  const isAllOnPageSelected = paginatedRows.every((r) =>
    selected.includes(r.id),
  );
  const isSomeOnPageSelected =
    paginatedRows.some((r) => selected.includes(r.id)) && !isAllOnPageSelected;
    ...
    <TableContainer component={Paper}>
      <Toolbar sx={{ justifyContent: "flex-start", gap: 2, flexWrap: "wrap" }}>
        ...
        <CategoryMenu
          ...
          categoryColumns={
            categoryColumns ?? columns.filter((c) => c.accessor !== "id")
          }
        />
      </Toolbar>
